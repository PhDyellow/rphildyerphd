...
#'
#' @return returns list containing a list of all clusterings as clara objects. Each clara object has been extended with an anova element from gf_anova().
...
gf_clust_f_ratio <- function(gf,
  ...
  cluster_fratio <- lapply(cluster_list, spatial_vars = spatial_vars, function(clust, spatial_vars){
    ...
    #cluster of samples
    sample_clust <- merge(data.frame(env_trans[, spatial_vars, drop = FALSE], clust = clust$clustering), gf_grid_sites, by = "site_id")
